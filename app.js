	...
	if (MDB){
		MongoClient.connect(MDB,function(err,db){
			...
			db.collectionNames(function(err,collections){
				res.render('main',{MDB:MDB, Collection: Collection, data1:showndata,collections: collections}); //doc
			});
		});
	} else {
...
app.post('/', function(req, res, next) {
		...
		if(req.body.action=="Show"){
			...
			insertdata(req.body.name,req.body.value,req.body.dataset,function(data) {
					showndata = data;
					res.redirect(req.url);
				});
		}
...
function insertdata(dbname,dbvalue,dbindex,callback){
	MongoClient.connect(MDB,function(err,db){
		...
		if(dbindex=="new")
			...
			db.collection(Collection).update({temp:"1234"},operator,options,function(err,upserted){
				if(err) throw err;
				showdata(function(data) {
						callback(data);
						return db.close();
			...
			finddocument(dbindex,function (que){
				...
				db.collection(Collection).update(query,operator,options,function(err,upserted){
					if(err) throw err;
					showdata(function(data) {
						callback(data);
						return db.close();
